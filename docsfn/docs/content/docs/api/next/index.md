...
| **`generateApiParams(manifest, { catchAll: true })`** | Overview and child routes as `{ slug?: string[] }[]` for `[[...slug]]`, with `{}` for the root overview; omit `catchAll` for slash-delimited strings |
